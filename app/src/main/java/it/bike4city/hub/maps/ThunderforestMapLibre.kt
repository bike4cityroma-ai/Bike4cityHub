...
fun ThunderforestMapLibre(
                ...
                mapView.apply {
                    getMapAsync { map ->
                        map.setStyle(buildThunderforestStyle(tfKey, style.id)) { loadedStyle ->
                            ...
                            updateTrack(loadedStyle, points)
                            if (initialBounds != null) {
                                map.moveCamera(CameraUpdateFactory.newLatLngBounds(initialBounds, 100))
                            } else if (points.isEmpty()) {
                                map.cameraPosition = org.maplibre.android.camera.CameraPosition.Builder()
                                    .target(org.maplibre.android.geometry.LatLng(initialCenter.latitude, initialCenter.longitude))
                                    .zoom(13.5)
                                    .build()
                            }
                        }
    ...
    LaunchedEffect(mapView, followMyLocation) {
        mapView.getMapAsync { map ->
            if (map.style?.isFullyLoaded == true && map.locationComponent.isLocationComponentActivated) {
                map.locationComponent.cameraMode = if (followMyLocation) {
                    map.animateCamera(CameraUpdateFactory.zoomTo(17.5))
    ...
    LaunchedEffect(style) {
        reloadStyle(mapView, tfKey, style, points)
    }

    LaunchedEffect(points) {
        mapView.getMapAsync { map ->
            map.style?.let { updateTrack(it, points) }
        }
    }
}
...
private fun reloadStyle(mapView: MapView, tfKey: String, style: TfStyle, points: List<LatLng>) {
    mapView.getMapAsync { map ->
        map.setStyle(buildThunderforestStyle(tfKey, style.id), Style.OnStyleLoaded {
            ensureTrackLayer(it)
            updateTrack(it, points)
        })
    }
}
...
private fun updateTrack(style: Style, points: List<LatLng>) {
    val src = style.getSourceAs<GeoJsonSource>("track-source") ?: return
    if (points.size < 2) {
        src.setGeoJson(FeatureCollection.fromFeatures(arrayOf()))
        return
    }
    val mlPoints = points.map { Point.fromLngLat(it.longitude, it.latitude) }
    val line = LineString.fromLngLats(mlPoints)
